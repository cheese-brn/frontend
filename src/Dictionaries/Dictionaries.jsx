...
const Dictionaries = () => {
  ...
  useEffect(() => {
    if (state.targetId) {
      let prefix
      if (viewTarget === OPEN_GENUSES) prefix = 'vid'
      else if (viewTarget === OPEN_PROPERTIES) prefix = 'subproperty'
      fetch(`/${prefix}/list/${state.targetId}`)
        .then((response) => response.json())
    ...
    <>
      <Paper sx={{ margin: '0 10px 0 10px', padding: '10px' }}>
        ...
        <div style={{ display: 'flex' }}>
          <DictionaryPage
            displayName="Рода"
            onClick={() => setViewTarget(OPEN_GENUSES)}
          />
          <DictionaryPage
            displayName="Свойства"
            onClick={() => setViewTarget(OPEN_PROPERTIES)}
          />
        </div>
        <div style={{ display: 'flex' }}>
        ...
        {state.targetId && (
          ...
          >
            <Typography variant="h5">
              {`Редактирование: ${dictionaryName} - ${
                tableContent.rows[state.targetId - 1].name
              }`}
            </Typography>
            {viewTarget === OPEN_PROPERTIES && (
              <>
                <Typography>Подсвойства:</Typography>
            ...
            {viewTarget === OPEN_GENUSES && (
              <>
                <Typography>Виды:</Typography>
                {childrenArray.map((type, index) => (
                  <p
                    key={`type-${index}`}
                  >{`${type.name} - ${type.childrenCount}`}</p>
                ))}
              </>
            )}
          </Paper>
